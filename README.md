...
<!-- Public Shields endpoint JSON under badges/ is generated by `cargo xtask badges`; verify with `cargo xtask badges --check`. `unsafe-review` badges are static review evidence: not memory-safety proof, not Miri-clean status, and not a UB-free claim. -->
