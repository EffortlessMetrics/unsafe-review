...
|---|---|---|---|
| Alignment guard | executable check like `is_aligned`, `align_offset`, modulo/equality check over same pointer before use | bare `align_of` mention; comment text; unrelated pointer; observed `is_aligned` value; closed positive `is_aligned` branch; checked pointer reassigned before use; post-use check | `raw_pointer_alignment`, `raw_pointer_alignment_is_aligned_guard`, `align_of_only_not_guard`, `alignment_other_pointer_not_guard`, `raw_pointer_alignment_post_check_not_guard`, `raw_pointer_alignment_observed_not_guard`, `raw_pointer_alignment_closed_branch_not_guard`, `raw_pointer_alignment_reassigned_pointer_not_guard`, `comment_alignment_not_guard` |
...
| Bounds guard | `len/capacity` relation executable and relevant to operation family | unrelated length variable; comment-only claim; bare predicate observation; post-access check; closed positive branch that no longer encloses the access; checked index reassigned before use; capacity observation without a guard; unrelated capacity comparison that does not bound the unsafe operation argument | `vec_set_len`, `raw_pointer_read_len_capacity_assert`, `get_unchecked_mut_len_guard`, `get_unchecked_mut_other_len_not_guard`, `get_unchecked_mut_post_check_not_guard`, `get_unchecked_mut_bounds_observed_not_guard`, `get_unchecked_mut_closed_bounds_not_guard`, `get_unchecked_mut_reassigned_index_not_guard`, `vec_set_len_capacity_observed_not_guard`, `vec_set_len_unrelated_capacity_comparison_not_guard` |
