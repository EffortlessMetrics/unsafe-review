...
| Bounds guard | `len/capacity` relation executable and relevant to operation family | unrelated length variable; comment-only claim; post-access check; capacity observation without a guard; unrelated capacity comparison that does not bound the unsafe operation argument | `vec_set_len`, `raw_pointer_read_len_capacity_assert`, `get_unchecked_mut_len_guard`, `get_unchecked_mut_other_len_not_guard`, `get_unchecked_mut_post_check_not_guard`, `vec_set_len_capacity_observed_not_guard`, `vec_set_len_unrelated_capacity_comparison_not_guard` |
