...
| Transmute valid-value guard | executable assertion or returning invalid-byte branch proving a narrow `u8` to `bool` byte domain before transmute | bare predicate observation, different argument, post-call check, or unsupported type pair | `transmute_bool_valid_value_guard`, `transmute_bool_invalid_return_guard`, `transmute_bool_value_observed_not_guard` |
