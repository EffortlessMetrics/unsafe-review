...
| UTF-8 validation | `str::from_utf8(buf).is_ok()`, a returning error path, or `str::from_utf8(buf)?` before `from_utf8_unchecked(buf)` | no validation, validation after use, or validation for a different buffer | `str_from_utf8_unchecked`, `str_from_utf8_unchecked_is_ok_guard`, `str_from_utf8_unchecked_is_err_return_guard`, `str_from_utf8_unchecked_question_mark_guard`, `str_from_utf8_unchecked_other_buffer_not_guard` |
