...
| Transmute valid-value guard | executable assertion, open positive branch containing the call, or returning invalid-byte branch proving a narrow `u8` to `bool` byte domain before transmute or transmute_copy | bare predicate observation, closed positive branch before the call, different argument, post-call check, or unsupported type pair | `transmute_bool_valid_value_guard`, `transmute_bool_invalid_return_guard`, `transmute_bool_value_observed_not_guard`, `transmute_bool_closed_if_observed_not_guard`, `transmute_copy_bool_value_observed_not_guard`, `transmute_copy_bool_closed_if_observed_not_guard` |
