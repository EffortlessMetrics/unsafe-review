...
| `ffi` | `unsafe extern "C"` declarations; foreign ABI surfaces | ffi_abi, ffi_ownership | `extern crate`; unsafe words in comments or strings | abi, ownership | address-sanitizer -> cargo-careful | `ffi_sanitizer_route` | imported operation paths and `extern crate` do not count as FFI boundary cards | ABI/layout and foreign-side ownership remain manual-review heavy |
