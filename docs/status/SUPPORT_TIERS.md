...
| Advisory baseline/suppression matching | experimental | policy ledgers / card class | xtask ledger tests plus analyzer tests for exact `baseline_known` and `suppressed` card identity matches | exact identity only; no broad suppressions, no no-new-debt, and no blocking policy |
