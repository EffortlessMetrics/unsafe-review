...
| Policy ledger schema validation | experimental | `cargo xtask check-policy` | xtask tests require baseline/suppression entries to carry exact counted card identity, owner, reason, evidence, and review or expiry dates | validates ledger shape only; no baseline matching, suppression application, no-new-debt, or blocking policy |
