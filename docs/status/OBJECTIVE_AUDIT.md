...
| Card correctness before breadth | Fixture goldens cover raw pointer alignment/deref/read/write, split syntax, inline unsafe operation dedupe, attributed unsafe-fn dedupe, public unsafe contracts including documented public unsafe API declarations, `MaybeUninit`, `Vec::set_len`, `Vec::set_len` initialized-loop evidence, `Vec::set_len` shrink evidence, `Vec::set_len` last-index shrink evidence, `Vec::set_len` start-bound shrink evidence, `Vec::set_len(0)` clear evidence, `transmute`, `get_unchecked_mut`, `Pin::new_unchecked`, `drop_in_place`, FFI, unsafe impl Send, and negative safe/comment cases; `fixtures/calibration.toml` indexes the core positive, negative, and false-positive-control claims | Experimental | Fixture corpus is curated; no broad semantic proof |
