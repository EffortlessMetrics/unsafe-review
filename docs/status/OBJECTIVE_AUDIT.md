...
- The `bytes#826` `Vec::from_raw_parts` site is now labeled as a Vec ownership
  ...
  narrow same-pointer `ManuallyDrop` raw-parts origin shape as ownership and
  len/capacity evidence. Allocator compatibility, layout, initialization, and
  broader ownership evidence remain source-level and advisory.
