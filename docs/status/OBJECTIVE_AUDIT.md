...
| First real-crate dogfood measurement | Handoff `2026-05-18-real-crate-dogfood-v0.6.md` records top-50 capped `rust-smallvec`, `arrayvec`, and `memchr` runs plus `memchr#215`, `rust-smallvec#407`, `rust-smallvec#254`, `arrayvec#308`, and `arrayvec#288` PR-diff runs; dogfood found and fixed import/declaration false positives, `cfg(target_feature)` false positives, capped repo scan timeout behavior, missing owner-contract inheritance for operation cards, comment-derived owner false positives, and fixture-backed `Vec::set_len` evidence improvements with `arrayvec#288` reruns | Experimental | More crates, more real PR diffs, uncapped/sampled runs, broader `Vec::set_len` evidence modeling, and human review are still needed before calibration claims |
