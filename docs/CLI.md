...
show the operation expression, operation family, and next action from the same
`ReviewCard`. It does not block, execute witnesses, or create broad suppression
authority.
...
expression for each planned comment and does not post comments.
